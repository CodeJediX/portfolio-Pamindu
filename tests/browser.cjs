...
await load('/',1440);
for(const [filter,count] of [['ai',6],['software',5],['web',2],['all',8]]){await p.locator(`[data-filter="${filter}"]`).click();assert.equal(await p.locator('.project:visible').count(),count)}report.interactions.push('Project filters: all four categories and counts passed');
...
for(const id of ['signspeak','sancharakaya','smart-face-door-lock','leaflens','magen','createx','signspeak-junior','hotel-management']){
