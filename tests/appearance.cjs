...
(async () => {
  ...
  assert.equal(await page.locator('.roadmap-stop').count(),3);
  assert.equal(await page.locator('.role-step').count(),6);
  ...
  report.checks.push('Three volunteering chapters and six positions; desktop/mobile screenshots');
